...
def run_intelligence_orchestrator(active_db_pl,
    ...
    if not active_db.empty:
        ...
        for target_id, data in engine_results.items():
            ...
            # ย่อชื่อจังหวัดให้สั้น เพื่อให้แสดงในตารางได้ครบ
            _prov_abbr = {
                'กรุงเทพมหานคร':'กทม','นครราชสีมา':'นม','เชียงใหม่':'ชม',
                'นนทบุรี':'นบ','สมุทรปราการ':'สป','ปทุมธานี':'ปท',
                'อุบลราชธานี':'อบ','ขอนแก่น':'ขก','อุดรธานี':'อด',
                'นครศรีธรรมราช':'นศ','สุราษฎร์ธานี':'สฎ','สงขลา':'สข',
                'ชลบุรี':'ชบ','ระยอง':'รย','จันทบุรี':'จบ','ตราด':'ตร',
                'นครปฐม':'นป','สุพรรณบุรี':'สพ','กาญจนบุรี':'กจ',
                'ราชบุรี':'รบ','เพชรบุรี':'พบ','ประจวบคีรีขันธ์':'ปข',
                'เลย':'ลย','หนองคาย':'นค','บึงกาฬ':'บก','นครพนม':'นพ',
                'มุกดาหาร':'มห','กาฬสินธุ์':'กส','ร้อยเอ็ด':'รอ',
                'ศรีสะเกษ':'ศก','สุรินทร์':'สน','บุรีรัมย์':'บร',
                'ชัยภูมิ':'ชย','อำนาจเจริญ':'อจ','ยโสธร':'ยส',
                'มหาสารคาม':'มค','สระแก้ว':'สก','ปราจีนบุรี':'ปจ',
                'นครนายก':'นย','ฉะเชิงเทรา':'ฉช','สระบุรี':'สบ',
                'ลพบุรี':'ลบ','สิงห์บุรี':'สห','ชัยนาท':'ชน',
                'อ่างทอง':'อท','พระนครศรีอยุธยา':'อย','อุทัยธานี':'อน',
                'นครสวรรค์':'นว','พิจิตร':'พจ','พิษณุโลก':'พล',
                'กำแพงเพชร':'กพ','ตาก':'ตก','สุโขทัย':'สท',
                'อุตรดิตถ์':'อต','แพร่':'พร','น่าน':'นน','พะเยา':'พย',
                'เชียงราย':'ชร','แม่ฮ่องสอน':'มส','ลำพูน':'ลพ','ลำปาง':'ลป',
            }
            def _shorten_plate(plate: str) -> str:
                for prov, abbr in _prov_abbr.items():
                    if plate.endswith(prov):
                        return plate[:-len(prov)] + abbr
                return plate
            _cars_display = " / ".join(_shorten_plate(str(c)) for c in data["cars"])

            priority_list.append({
                "Target_ID": target_id,
                "เป้าหมาย": _cars_display,
                "ประเภท": final_type,
