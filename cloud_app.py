...
def run_intelligence_orchestrator(active_db_pl,
    ...
    if not active_db.empty and 'Speed_kmh' in active_db.columns:
        ...
        def _ocr_error_score(plate, df_target):
            ...
            # Filter 5: กล้องเดิมแจ้งซ้ำมาก — กล้องเดียวพบทะเบียนนี้เยอะผิดปกติ (+15)
        ...
        def _ocr_historical_adjust(plate, ocr_score):
            ...
            ถ้าทะเบียนไม่เคยปรากฏในประวัติเลย (+10 เพิ่มโอกาสเป็น OCR Error)
            ถ้ามีประวัติยาวนานหลายสัปดาห์ (-10 ลดโอกาส OCR Error)"""
                        ...
                        if _rep.empty:
                            ...
                            def _show_repeat_table(group_df, tab_key, icon):
                                ...
                                excel_download_button(
                                    build_2col_export_df(group_df), f"repeat_{tab_key}_{selected_date}.xlsx",
                                    "📥 Export รายชื่อทะเบียน (Excel)"
                                )
